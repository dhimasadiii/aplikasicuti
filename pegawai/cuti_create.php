						...
						<form class="form-horizontal" name="cuti" action="cuti_insert.php" method="POST" enctype="multipart/form-data" onSubmit="return valid();">
							<div class="panel panel-default">
								...
								<div class="panel-body">
									...
									<div class="form-group">
										<label class="control-label col-sm-3">Pilihan Cuti</label>
										<div class="col-sm-4">
										<select class="form-control" name="akses" required>
											<option value="" class="text-center">Pilihan Cuti</option>
											<option value="Admin">Cuti Site</option>
											<option value="Mng">Cuti Tahunan</option>
											<option value="Spv">Cuti Besar</option>
											</select>		
									</div>
									</div>
									<div class="form-group">
										...
										<div class="col-sm-4">
											...
											<option value="" class="text-center" selected>Pilih Supervisor</option>
												<?php
													$mySql = "SELECT * FROM employee WHERE hak_akses='Supervisor' AND active='Aktif' ORDER BY nama_emp";
													$myQry = mysqli_query($conn, $mySql);
													$dataSupervisor = $result['npp'];
													while ($supervisorData = mysqli_fetch_array($myQry)) {
														if ($supervisorData['npp']== $dataSupervisor) {
														$cek = " selected";
														} else { $cek=""; }
														echo "<option value='$supervisorData[npp]' $cek>".$supervisorData[nama_emp]."</option>";
													}
												?>
											</select>
										</div>
